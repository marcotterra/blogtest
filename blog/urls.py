...
urlpatterns = [
    ...
    #url(r'^page/(?P<page>\d+)/$', 'BEDetailView.as_view()',  name='blog_page'),
    
    url(r'^(?P<year>\d{4})/(?P<month>\d+)/(?P<slug>[-\w]+)/$', BlogEntryDetail.as_view(), name='blogentry-detail'),
    url(r'^author/(?P<username>[\w.@+-]+)/$', AuthorView.as_view(), name='author-detail'),
